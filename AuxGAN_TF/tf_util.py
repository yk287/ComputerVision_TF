...
def classification_loss(logits_real, logits_fake, class_array):
    """Compute the GAN loss.
    ...
    its two components, and then add them together (instead of averaging once at the very end).
    """
    G_loss = tf.reduce_sum(tf.losses.mean_squared_error(labels=class_array, predictions=logits_fake))
    D_loss = tf.reduce_sum(tf.losses.mean_squared_error(labels=class_array, predictions=logits_real))
