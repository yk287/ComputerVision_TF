...
def recon_loss(input_image, recon_image):
    ...
    #recon_loss = tf.reduce_mean(tf.losses.(labels=input_image, predictions=recon_image))
    recon_loss = tf.reduce_sum(tf.nn.sigmoid_cross_entropy_with_logits(labels=input_image, logits=recon_image), axis=[1])
    return recon_loss
